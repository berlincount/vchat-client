...
char const *own_nick_get() { return g_nick; }
...
int own_channel_get() { return g_channel; }
...
int ul_add(char *name, int in_my_chan_flag) {
  ...
  case 1:
    g_users[base].flags |= UL_IN_MY_CHAN;
    break;
  case 0:
    g_users[base].flags &= ~UL_IN_MY_CHAN;
    break;
...
int ul_del(char *name) {
  ...
  /* Copy the tail */
  memmove(g_users + base, g_users + base + 1,
          (g_users_count - base - 1) * sizeof(user));
...
static int ul_compare_private(const void *a, const void *b) {
  ...
  if (_a->last_private > _b->last_private)
    return -1;
  return 1;
}
...
static int ul_compare_begin_of_line_ncase(const void *a, const void *b) {
  ...
  if (!(_a->flags & UL_IN_MY_CHAN))
    return 1;
  if (!(_b->flags & UL_IN_MY_CHAN))
    return -1;
  ...
  if (a_i && b_i)
    return 0; // Both nicks dont match
  if (!a_i && b_i)
    return -1; // a matches insensitive, b doesnt
  if (a_i && !b_i)
    return 1; // b matches insensitive, a doesnt
  ...
  if (_a->last_public > _b->last_public)
    return -1;
...
static int ul_compare_begin_of_line_case(const void *a, const void *b) {
  ...
  if (a_i && b_i)
    return 0; // Both nicks dont match at all
  if (!a_i && b_i)
    return -1; // a matches insensitive, b doesnt
  if (a_i && !b_i)
    return 1; // b matches insensitive, a doesnt
  ...
  if (!a_s && b_s)
    return -1; // a matches sensitive, b doesnt
  if (a_s && !b_s)
    return 1; // b matches sensitive, a doesnt
...
char **ul_complete_user(char *text, int start, int end) {
  ...
  if (start == 0 && end == 0) {
    ...
    /* Copy common prefix */
    if (result_count)
      result[0] = strndup(text, end - start);
