...
static void forceredraw_wrapper(int a) {
  (void)a;
  forceredraw();
}
...
static int getsbeheight(struct sb_entry *entry, const int xwidth,
                        int needstime);
static int getsbdataheight(struct sb_data *data, const int xwidth,
                           int needstime);
/* CURRENTLY UNUSED
...
static int calcdrawcus(char *const str) {
  char *tmp = str;
  int zero = 0;
  while (*tmp && (*tmp != ' ') && (*tmp != '\n')) {
    if (*tmp == 1)
...
static void sb_flush(struct sb_data *sb) {
...
static struct sb_entry *sb_add(struct sb_data *sb, const char *line,
  ...
  if (newone) {
    ...
    newone->stamp = 0xffff;
    if (sb->entries)
      sb->entries->link = (struct sb_entry *)((unsigned long)sb->entries->link ^
                                              (unsigned long)newone);
    else
      sb->last = newone;
    sb->entries = newone;
  }
...
void writeout(const char *str) {
  ...
  if (i > outputwidth_desired)
    outputwidth_desired = i;
}
...
#define WATTR_GET(win, orgattr)                                                \
  wattr_get(win, &orgattr.attr, &orgattr.pair, NULL)
...
#define BCOLR_SET(attr, colour)                                                \
  attr->attr = ((attr->attr) & ~A_COLOR) | COLOR_PAIR((colour));

#endif

/* 'A' - 'Z' attriute type */
static int attributes[] = {A_ALTCHARSET,
                           A_BOLD,
                           0,
                           A_DIM,
                           0,
...
static void docolorize(char colour, ncurs_attr *attr, ncurs_attr orgattr) {
  if (colour == '0') {
    *attr = orgattr;
    ...
    if ((upc >= 'A') && (upc <= 'Z') && (newattr = attributes[upc - 'A']))
      attr->attr =
          (colour & 0x20) ? attr->attr | newattr : attr->attr & ~newattr;
  }
}
...
static int writescr(WINDOW *win, struct sb_entry *entry) {
  ...
  for (i = 0; i < charcount; i++) {
    ...
    if (textbuffer[i] == ' ') {
      if ((calcdrawcus(textbuffer + i + 1) + getcurx(win) >
           getmaxx(win) - 1 - 1) &&
...
static void resize_output() {
  int outputwidth =
      (outputwidth_desired + 7 > screensx) ? screensx - 7 : outputwidth_desired;
  int outputheight = getsbdataheight(sb_out, outputwidth - 1, 0);

  if (outputheight + 5 > screensy)
    outputheight = screensy - 5;
  ...
  togglequery();
  {
    ...
    if (sb->scroll != sb->count)
      while (now && (now->id != sb->scroll)) {
        tmp = now;
        now =
            (struct sb_entry *)((unsigned long)now->link ^ (unsigned long)prev);
        prev = tmp;
      }
    ...
    if (!up) {
      prev =
          (struct sb_entry *)((unsigned long)now->link ^ (unsigned long)prev);
      tmp = now;
      now = (struct sb_entry *)((unsigned long)now->link ^ (unsigned long)prev);
      prev = tmp;
    }

    while (now && (lines > 0)) {
      if ((!filtertype) ||
          ((now->stamp != currentstamp) &&
...
void resize(int signal) {
  ...
  if (screensy - privheight < 4) {
    fprintf(stderr,
            "vchat-client: screen to short (only %d rows, at least %d "
            "expected), bailing out.\n",
  ...
  if (screensx < 14) {
    fprintf(stderr,
            "vchat-client: screen to thin (only %d cols, at least %d "
            "expected), bailing out.\n",
            screensx, 14);
    snprintf(errstr, ERRSTRSIZE,
             "vchat-client: screen to thin (only %d cols, at least %d "
...
static int getsbeheight(struct sb_entry *entry, const int xwidth,
  ...
  if (needstime) {
    int timelen =
        (int)strftime(tmp, 64, getformatstr(FS_TIME), localtime(&entry->when));
    tmp[timelen] = 2;
    textbuffer = tmp;
  } else {
...
*/
static int analyzefilters(void) {
...
void topicline(char *message) {
  ...
  if (priv) {
    ...
    if (sb_win == 1)
      mvwaddch(topic, 0, getmaxx(topic) - 1, '*');
  }
...
/* Filter stuff */
static int check_valid_colour(char colour) {
...
*/
static int removefromfilterlist(int (*test)(filt *flt, void *data, char colour),
                                void *data, char colour) {
...
static int test_clear(filt *flt, void *data, char c) {
  (void)data;
  if (!c || (c == flt->colour) ||
      ((c == '*') && (flt->colour != '-') && (flt->colour != '+')))
    return RMFILTER_RMANDCONT;
  else
    return RMFILTER_KEEPANDCONT;
}

static int test_simplerm(filt *flt, void *data, char colour) {
...
static int test_numericrm(filt *flt, void *data, char colour) {
  if (flt->id == (long)data)
    return test_clear(flt, NULL, colour);
  else
    return RMFILTER_KEEPANDCONT;
...
unsigned int addfilter(char colour, char *regex) {
  ...
  *flt = newflt;
  ...
  if (colour == '-') {
    snprintf(tmpstr, TMPSTRSIZE,
             "  \"%s\" successfully added to ignorance list. ( ID = %d). ",
             (*flt)->text, (*flt)->id);
  } else if (colour == '+') {
    snprintf(tmpstr, TMPSTRSIZE,
             "  \"%s\" successfully added to zoom list. ( ID = %d). ",
             (*flt)->text, (*flt)->id);
  } else {
    snprintf(tmpstr, TMPSTRSIZE,
             "  \"%s\" successfully added to hilitelist. (ID = %d). ",
             (*flt)->text, (*flt)->id);
  }
  writeout(tmpstr);
  showout();
...
void listfilters(void) {
  ...
  while (flt) {
    if ((flt->colour != '-') && (flt->colour != '+')) {
      ...
      snprintf(tmpstr, TMPSTRSIZE, "  ID: [% 3d] Color: [%c] Regex: [%s]",
               flt->id, flt->colour, flt->text);
      writeout(tmpstr);
    }
    flt = flt->next;
  }
  ...
  while (flt) {
    if (flt->colour == '-') {
      ...
      snprintf(tmpstr, TMPSTRSIZE, "  ID: [% 3d]            Regex: [%s]",
               flt->id, flt->text);
      writeout(tmpstr);
    }
    flt = flt->next;
